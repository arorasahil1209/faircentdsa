...
let uploadS3Docs = async (req, res) => {
  try {
    ...
    datas.append(
      "fileKey",
      fs.createReadStream(`${path.resolve("uploads")}/${req.file.originalname}`)
    );
    ...
    if (req.body.docType == "selfie") {
      getDocId = await db.sequelize.query(`${query} '${req.body.docType}'`, {
        raw: true,
      });
...
let downloadDocument = async (req, res) => {
  try {
    ...
    let config = {
      method: "post",
      url: Config.faircentApi.url + "/standard/download",
      data: {
